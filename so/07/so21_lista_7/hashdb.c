...
static void db_open(db_t *db, const char *name, size_t size) {
  ...
  db->entry = Mmap(NULL, size * sizeof(entry_t), DB_PROT, MAP_SHARED, fd, 0);
  db->size = size;
  db->name = strdup(name);
...
static void doit(const char *path, op_t mode) {
  ...
  if (isatty(STDIN_FILENO) || S_ISFIFO(sb.st_mode)) {
    ...
    char *buf = Mmap(NULL, sb.st_size, DB_PROT, MAP_PRIVATE, STDIN_FILENO, 0);
    while ((buf = consume_line(buf, &db, mode)));
  }
