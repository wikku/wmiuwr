...
Lemma H_Consequence_post_True : forall (P Q : Assertion) c,
    derivable P c Q ->
    derivable P c True.
Proof.
  intros.
...
Proof.
  induction c; intros.
  - eapply H_Consequence_post_True.
    constructor.
  - eapply H_Consequence; unfold assert_of_Prop; intros.
    + constructor.
    + assert (True [x |-> a] st).
      * unfold assertion_sub. simpl. exact I.
      * exact H0.
    + exact I.
  - specialize (IHc1 P) as P1.
    specialize (IHc2 True) as P2.
    eapply H_Seq.
    + exact P2.
    + exact P1.
  - specialize (IHc1 (P /\ b)%assertion) as P1.
    specialize (IHc2 (P /\ ~b)%assertion) as P2.
    eapply H_If.
    + exact P1.
    + exact P2.
...
Lemma H_Consequence_pre_False : forall (P Q : Assertion) c,
    derivable P c Q ->
    derivable False c Q.
Proof.
  intros.
...
Proof.
  induction c; intros.
  - eapply H_Consequence_pre_False.
    constructor.
  - eapply H_Consequence_pre_False.
    constructor.
  - eapply H_Consequence_pre_False.
  ...
  - eapply H_If.
    + eapply H_Consequence_pre.
      * apply IHc1.
      * intros st [fals _]. destruct fals.
    + eapply H_Consequence_pre.
      * apply IHc2.
      * intros st [fals _]. destruct fals.
  - eapply H_Consequence_pre_False.
    eapply H_Consequence_post.
    eapply H_While with (P := fun _ => False).
    eapply H_Consequence_pre.
    apply IHc.
    + intros st [fals _].
      destruct fals.
...
Proof.
  ...
  - eapply H_Consequence_pre.
    ...
    + auto.
  - eapply H_Consequence.
    eapply (H_While (wp <{while b do c end}> _)).
    + apply IHc.
      intros st st' ev.
      apply wp_invariant.
      apply ev.
    ...
    + simpl.
      intros st [P1 P2].
      eapply (wp_is_precondition <{ while b do c end }> Q st).
      * apply E_WhileFalse.
        destruct (beval st b).
        ** exfalso. apply P2. auto.
        ** auto.
      * exact P1.
Qed.
