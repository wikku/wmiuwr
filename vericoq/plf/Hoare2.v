  ...
  <{
    {{ True }}
      ...
      Z := 1
                {{ X = 0 /\ Y = 1 /\ Z = 1 }};
      while X <> n do
                {{ 2*Z = ap pow2 (1+X) /\ Y+2*Z = ap pow2 (2+X) - 1 }}
        Z := 2 * Z
                {{ Z = ap pow2 (1+X) /\ Y+Z = ap pow2 (2+X) - 1 }};
        Y := Y + Z
                {{ Z = ap pow2 (1+X) /\ Y = ap pow2 (2+X) - 1 }};
        X := X + 1
                {{ Z = ap pow2 X /\ Y = ap pow2 (1+X) - 1 }}
      end
    {{ Y = pow2 (n+1) - 1 }}
  }>.
...
Definition dfib (n : nat) : decorated :=
  <{
    ...
    while X <> 1 + n do
      ...
      T := Z
                  {{ X <> 0 /\  Y = ap fib (X-1) /\ Z = ap fib X /\ T = ap fib X }} ;
      Z := Z + Y
                  {{ X <> 0 /\ Z = ap fib (1+X) /\ T = ap fib X }} ;
      Y := T
                  {{ X <> 0 /\ Y = ap fib X /\ Z = ap fib (1+X) }} ;
      X := 1 + X
                  {{ X <> 0 /\ Y = ap fib (X-1) /\ Z = ap fib X }}
    end
    {{ Y = fib n }}
...
Proof.
  ...
  - destruct (st X).
    ...
    + simpl sub. rewrite sub_0_r. reflexivity.
  - rewrite sub_0_r. reflexivity.
Qed.
...
| DCWhile (b : bexp) (d : dcom) (P : Assertion).
...
Fixpoint erase (d : dcom) : com :=
  ...
  | DCWhile b d _ => CWhile b (erase d)
  end.
...
Fixpoint verification_conditions (P : Assertion) (d : dcom) (Q : Assertion) : Prop :=
  ...
  | DCWhile b d L =>
      (P ->> L)
      /\ verification_conditions (L /\ b) d L
      /\ ((L /\ ~b) ->> Q)%assertion
  end.

Theorem verification_correct : forall d P Q,
  verification_conditions P d Q -> {{P}} erase d {{Q}}.
Proof.
  induction d; intros; simpl in *.
  - eapply hoare_consequence_pre.
    + apply hoare_skip.
