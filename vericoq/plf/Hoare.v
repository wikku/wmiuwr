...
Proof.
  ...
  exists <{X + 1}>.
  intros.
  specialize H with (X !-> 0) (X !-> 1; X !-> 0).
  assert (Aexp_of_aexp X (X !-> 1) = Aexp_of_aexp <{ X + 1 }> (X !-> 1)).
  - apply H.
    + constructor. auto.
    + unfold assert_of_Prop. auto.
  - unfold t_update in H0. simpl in H0. discriminate.
Qed.
...
Proof.
  ...
  inversion H. subst.
  exists (aeval st X).
  assert ((X !-> aeval st X; X !-> aeval st a; st) = st).
  - apply FunctionalExtensionality.functional_extensionality.
    intros.
    unfold t_update.
    destruct (String.eqb_spec X x).
...
Proof.
  unfold valid_hoare_triple.
  intros H.
  specialize H with (a := X) (n := 0) (st := (X !-> 0)).
  eassert (H := H ?[st']).
  assert ((X !-> 0) =[ X := 3; Y := X ]=> ?st').
  - eapply E_Seq.
    + apply E_Asgn. auto.
    + apply E_Asgn. auto.
  - apply H in H0.
...
Inductive ceval : state -> com -> state -> Prop :=
  ...
  | E_WhileTrue : forall st st' st'' b c,
      ...
      st' =[ while b do c end ]=> st'' ->
      st  =[ while b do c end ]=> st''
  | E_RepeatTrue : forall b st c st',
      st  =[ c ]=> st' ->
      beval st' b = true ->
      st  =[ repeat c until b end]=> st'
  | E_RepeatFalse : forall b st c st' st'',
      st  =[ c ]=> st' ->
      beval st' b = false ->
      st' =[ repeat c until b end]=> st'' ->
      st  =[ repeat c until b end]=> st''
...
Proof.
  intros.
  remember <{repeat c until b end}> as rc.
  intros st st' Hev HP.
  induction Hev;
    try (inversion Heqrc; subst);
    eauto.
  - apply IHHev2; try auto.
    apply H0. split.
    + apply (H st st'); auto.
    + unfold not. intro. unfold bassertion in H2. rewrite H1 in H2. discriminate.
Qed.
...
Proof.
  ...
  - apply hoare_havoc.
  - intros st Pst n. simpl.
    unfold assertion_sub. simpl.
    exists (aeval st X).
    assert ((X !-> aeval st X; X !-> aeval st n; st) = st).
    + apply FunctionalExtensionality.functional_extensionality.
      intros.
      unfold t_update.
      destruct (String.eqb_spec X x).
...
Proof.
  ...
  exists True%assertion.
  split.
  - intros st r ev Pst. inversion ev. subst.
    simpl in H0. rewrite eqb_refl in H0. simpl in H0. discriminate.
  - intro contra. unfold valid_hoare_triple in contra.
    Search "empty".
    specialize (contra empty_st RError).
    assert (exists st' : state, RError = RNormal st' /\ assert_of_Prop True st').
    + apply contra.
      * constructor. auto.
      * auto.
...
Proof.
  intros.
  intros st st' Hev Pst.
  specialize (H st st').
  apply H.
  - inversion Hev. subst. constructor. assumption.
  - assumption.
Qed.
...
Theorem hoare_assert : forall P b,
  ...
  {{ P }} assert b {{ P }}.
Proof.
  intros P b Pb st st' Hev Pst.
  inversion Hev; subst.
  - exists st. auto.
  - specialize (Pb st Pst). rewrite Pb in H0. discriminate.
Qed.
...
Proof.
  ...
  - eapply hoare_seq.
    + eapply hoare_consequence_post.
      * apply (hoare_assert). instantiate (1 :=<{X = 2}>). auto.
      * auto.
    + apply hoare_asgn.
  - eapply hoare_consequence_post.
    + apply hoare_assume.
