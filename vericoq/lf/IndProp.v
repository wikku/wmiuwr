...
Proof.
  split.
  {
    ...
    - apply ev_0.
    - apply (ev_SS _ ev_0).
    - apply (ev_sum _ _ IHev'1 IHev'2).
  }
  {
    intros.
    induction H.
...
Proof.
  ...
  induction H0.
  - intros. exact H.
  - intros. simpl in H. inversion H. apply (IHev H2).
Qed.
(** [] *)
...
Inductive subseq : list nat -> list nat -> Prop :=
  ...
  | subseq_hd : forall n xs ys, subseq xs ys -> subseq (n :: xs) (n :: ys)
  | subseq_skip : forall n xs ys, subseq xs ys -> subseq xs (n :: ys)
.
...
Proof.
  ...
  induction H.
  - apply subseq_nil.
  - simpl. apply subseq_hd. exact IHsubseq.
  - simpl. apply subseq_skip. exact IHsubseq.
Qed.
...
Proof.
  ...
  induction H0; intros.
  - inversion H. apply subseq_nil_nil.
  - inversion H.
    + apply subseq_hd. apply IHsubseq. exact H3.
    + apply subseq_skip. apply IHsubseq. exact H3.
  - apply subseq_skip. apply IHsubseq. exact H.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. apply MStarApp.
    ...
    + apply IHss. intros. apply H. right. exact H0.
Qed.
(** [] *)
...
Proof.
  split.
  {
    induction re; intros; destruct H.
    - inversion H.
    - reflexivity.
    - reflexivity.
    - simpl. inversion H; subst. rewrite IHre1. rewrite IHre2.
      + reflexivity.
      + exists s2. exact H4.
      + exists s1. exact H3.
...
Proof.
  ...
  - simpl. intros le.
    ...
    + apply IH1 in H. destruct H. destruct H. destruct H. destruct H.
      ...
      rewrite app_assoc.
      rewrite app_assoc.
      rewrite <- (app_assoc _ x (napp m x0) x1).
      apply MApp.
      apply H1.
      exact Hmatch2.
  ...
  - simpl. intro le.
    ...
    simpl in IH2.
    destruct s1.
    + simpl. simpl in le. apply IH2. exact le.
...
Proof.
  ...
  - unfold not. intros. exact H.
  - unfold not. intros.
    simpl in Hcount.
    simpl in H.
    destruct (eqbP n m).
    + discriminate.
    + destruct H. symmetry in H. apply H0. exact H.
      apply IHl' in Hcount. apply Hcount. exact H.
Qed.
(** [] *)
