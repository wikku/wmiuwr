...
Proof.
  split.
  {
    ...
    induction b;
      ...
      try reflexivity;
      subst;
      try (apply aeval_iff_aevalR in H2; apply aeval_iff_aevalR in H4; subst);
      try reflexivity.
  ...
  {
    generalize dependent bv.
    induction b; intros; unfold beval in H;
      subst; constructor;
...
Proof.
  ...
  - apply E_WhileTrue with (st' := (X !-> 1 ; Y !-> 2; Y !-> 0 ; X !-> 2)).
    ...
    + apply E_WhileTrue with (st' := (X !-> 0 ; Y !-> 3 ; X !-> 1 ; Y !-> 2; Y !-> 0 ; X !-> 2)).
      * reflexivity.
      * apply E_Seq with (st' := (Y !-> 3 ; X !-> 1 ; Y !-> 2; Y !-> 0 ; X !-> 2)).
        { apply E_Asgn. reflexivity. }
        { apply E_Asgn. reflexivity. }
...
Proof.
  ...
  - apply IHcontra2. exact Heqloopdef.
Qed.
(** [] *)
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_WhileTrueBreak : forall st st' b c,
      ...
      st  =[ c ]=> st' / SBreak ->
      st  =[ while b do c end ]=> st' / SContinue
  | E_WhileTrue : forall st st' st'' b c r,
      beval st b = true ->
      st  =[ c ]=> st' / SContinue ->
...
Proof.
  intros.
  remember <{while b do c end}> as c' in H.
...
Proof.
  intros.
  apply E_WhileTrueBreak with (st' := st').
  - assumption.
  - assumption.
Qed.
...
Proof.
  intros.
  remember <{while b do c end}> as wc.
  induction H; try discriminate; inversion Heqwc; subst.
...
Proof.
  ...
  induction H1; intros s2 st2 H2; inversion H2; subst; auto.
  - apply IHceval in H3. destruct H3. discriminate.
  - apply IHceval1 in H5. destruct H5. discriminate.
  - apply IHceval1 in H1. destruct H1. subst.
  ...
  - apply IHceval in H8. destruct H8. auto.
  - apply IHceval in H5. destruct H5. discriminate.
  - rewrite H in H6. discriminate.
  - apply IHceval1 in H7. destruct H7. discriminate.
  - apply IHceval1 in H4. destruct H4. subst.
    apply IHceval2 in H8. assumption.
Qed.
(** [] *)
End BreakImp.
