...
Proof.
  ...
  - simpl. rewrite IHb. rewrite mul_comm. rewrite <- mult_n_Sm.
    assert (S (bin_to_nat b * 2 + 1) = 2 * bin_to_nat b + 2).
    rewrite add_comm.
    assert (S (1 + bin_to_nat b * 2) = S (S (bin_to_nat b * 2))).
    reflexivity.
    rewrite H.
    rewrite add_comm.
    rewrite mul_comm.
...
Example normalize_0z : normalize (B0 Z) = Z.
Proof. reflexivity. Qed.
Example normalize_00z : normalize (B0 (B0 Z)) = Z.
Proof. reflexivity. Qed.
Example normalize_10z : normalize (B1 (B0 Z)) = B1 Z.
Proof. reflexivity. Qed.
...
Proof.
  ...
  - reflexivity.
  - simpl.
    assert (forall n, nat_to_bin (n * 2) = double_bin (nat_to_bin n)).
    intros. induction n.
    reflexivity.
    simpl. rewrite IHn. rewrite double_incr_bin. reflexivity.
    rewrite H. rewrite IHb. reflexivity.
  - simpl.
    assert (forall n, nat_to_bin (n * 2 + 1) = B1 (nat_to_bin n)).
    intros. induction n.
    reflexivity.
