...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => nil
  | n :: l => if odd n then n :: oddmembers l else oddmembers l
  end.

Check oddmembers [1;3].
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  match s with
  | nil => nil
  | x :: xs => if x =? v then xs else x :: remove_one v xs
  end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | x :: xs => member x s2 && included xs (remove_one x s2)
  end.
