...
Proof.
  ...
  - intros [HP | HPNR].
    + split.
      ...
      { left. exact HP. }
    + split.
      { case HPNR. intros HQ HR. right. exact HQ. }
      { case HPNR. intros HQ HR. right. exact HR. }
  - intros [HPvQ HPvR].
    case HPvQ.
    + intros HP. left. exact HP.
    + case HPvR.
      { intros HP HQ. left. exact HP. }
      { intros HR HQ. right. apply (conj HQ HR). }
Qed.
(** [] *)
...
Proof.
  ...
  - intros. simpl. destruct m.
    + discriminate H.
    + simpl in H. apply IHn in H. destruct H. exists x. f_equal. exact H.
Qed.
...
Proof.
  induction n.
  - intros. reflexivity.
  - intros. destruct H. rewrite H. simpl. apply IHn. exists x. reflexivity.
Qed.
...
Proof.
  ...
  - induction l.
    ...
    + intros. simpl. split.
      ...
      * apply IHl. intros. apply H. simpl. right. exact H0.
  - induction l.
    + intros. destruct H0.
    + intros. destruct H0.
      * destruct H. destruct H0. exact H.
      * destruct H. apply (IHl H1 _ H0).
Qed.
(** [] *)
...
Proof.
  split.
  {
    ...
    - discriminate H0.
    - simpl in H0. f_equal.
      + apply H. destruct (eqb x x0).
        * reflexivity.
        * discriminate H0.
      + apply IHl1. destruct (eqb x x0).
        * exact H0.
        * discriminate H0.
  }
  {
    intros. destruct H0. induction l1.
    - reflexivity.
    - simpl. assert (eqb x x = true).
      rewrite H. reflexivity.
      rewrite H0. simpl. exact IHl1.
  }
Qed.
...
Proof.
  induction l1; intros.
  - reflexivity.
  - simpl. rewrite <- app_assoc. apply IHl1.
Qed.
...
Proof.
  ...
  unfold tr_rev.
  intros.
  rewrite <- (app_nil_r _ (rev x)).
  apply rev_append_correct.
Qed.
(** [] *)
