...
\section*{4/6}
Najdłuższy wspólny podciąg $x[1..n]$ i $y[1..m]$ …

\subsection*{zawierający podciąg}
Niech $L[i,j,k]$ to najdłuższy wspólny podciąg $x[1..i]$ i $y[1..j]$ zawierający
podciąg $z[1..k]$.
Wtedy $L[i,j,k] = $
$$
\begin{cases}
	ε & \text{gdy }k=0 \text{ i } i=0\text{ lub }j=0 \\
	\text{NULL} & \text{gdy }k>0 \text{ i } i=0\text{ lub }j=0 \\
	L[i-1,j-1,k-1]x[i] & \text{gdy }i,j,k>0\text{ i }x[i]=y[j]=z[k]\\
	L[i-1,j-1,0]x[i] & \text{gdy }i,j>k=0\text{ i }x[i]=y[j] \\
	L[i-1,j-1,k]x[i] & \text{gdy }i,j,k>0\text{ i }x[i]=y[j]≠z[k]\\
	\max\{L[i-1,j,k], L[i,j-1,k]\} & \text{gdy }i,j>0\text{ i }x[i]≠y[j]
\end{cases}
$$

\subsection*{nie zawierający podciągu}
Niech $L[i,j,k]$ to najdłuższy wspólny podciąg $x[1..i]$ i $y[1..j]$ nie zawierający
podciągu $z[1..k]$.
Wtedy $L[i,j,k] =$
$$
\begin{cases}
	ε & \text{gdy }k>0 \text{ i } i=0\text{ lub }j=0 \\
	L[i-1,j-1,k] & \text{gdy }i,j≥k=1\text{ i }x[i]=y[j]=z[k]\\
	\max\{L[i-1,j-1,k], \\ \qquad\, L[i-1,j-1,k-1]x[i]\} & \text{gdy }i,j>0, k>1\text{ i }x[i]=y[j]=z[k]\\
	L[i-1,j-1,k]x[i] & \text{gdy }i,j,k>0 \text{ i }x[i]=y[j]≠z[k]\\
	\max\{L[i-1,j,k], L[i,j-1,k]\} & \text{gdy }i,j>0\text{ i }x[i]≠y[j]
\end{cases}
$$

\subsection*{zawierający podsłowo}

\subsection*{nie zawierający podsłowa}
