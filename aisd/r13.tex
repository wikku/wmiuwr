...
\section*{7/7}
Próby w poszukiwaniu elementu zakończonym sukcesem to dokładnie
próby wykonane podczas wstawiania tego elementu.
Z równym prawdopodobieństwem będziemy wyszukiwać dowolny element
(w kolejności wstawiania),
więc aby obliczyć oczekiwany koszt wyszukiwania,
obliczymy oczekiwany koszt wszystkich wstawień i podzielimy przez $n$.

Prawdopodobieństwo, że przy wstawianiu $i$-tego (indeksując od 0) elementu
będziemy potrzebowali co najmniej $j$ prób wynosi
$$∏_{k=0}^{j-1} \frac{i-k}{m-k} ≤ \left(\frac{i}{m}\right)^j.$$
Zatem oczekiwaną liczbę prób przy jego wstawianiu możemy oszacować przez
$$∑_{j=0}^∞ \left(\frac{i}{m}\right)^j ≤ \frac{1}{1- \frac{i}{m} }.$$

Oczekiwany koszt dowolnego wstawienia to w takim razie
\begin{align*}
	\frac{1}{n} ∑_{i=0}^n \frac{1}{1- \frac{i}{m} }
	&=\frac{1}{n} ∑_{i=0}^n \frac{m}{m-i}
	=\frac{m}{n} ∑_{i=0}^n \frac{1}{m-i}
	=\frac{m}{n} ∑_{i=m-n+1}^m \frac{1}{i} \\
	&≤\frac{1}{α} ∫_{m-n}^m \frac{1}{x}\,dx
	=\frac{1}{α} \ln \frac{m}{m-n}
	=\frac{1}{α} \ln \frac{1}{1-α}.
\end{align*}
