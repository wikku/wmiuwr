...
\section*{6/7a}
% Udowodnić, że rozszerzenie $K ⊂ K(X,Y)$ jest czysto przestępne.

\iffalse
Musimy pokazać, że każdy element $a ∈ K(X,Y) ∖ K$ jest przestępny, tzn.
nie jest pierwiastkiem żadnego wielomianu z $K[T]$.

% $K ⊆ K(X) ⊆ K(X,Y)$
% $K(a)$
\fi

Weźmy $a ∈ K(X,Y)$ i wielomian $∑_{i=0}^n c_iT^i = f ∈ K[T]$ taki,
że $f(a) = 0$.
Element $a$ możemy zapisać jako $p/q$, gdzie $p,q ∈ K[X,Y]$ względnie pierwsze.
Wtedy z twierdzenia o pierwiastku wymiernym $p$ dzieli $c_0$ i $q$ dzieli $c_n$,
więc $p,q,a ∈ K$.

Zatem każdy element $K(X,Y) ∖ K$ jest przestępny nad $K$, czyli jest to
